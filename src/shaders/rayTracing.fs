...
void initRNG()
{
    uint spatialSeed = uint(gl_FragCoord.x) * 1973u
                     + uint(gl_FragCoord.y) * 9277u;
    uint temporalSeed = uint(frameID) * 26699u;
    uint seed = spatialSeed ^ temporalSeed ^ 12345u;

    m_u = seed;
    m_v = seed ^ 0x68bc21ebu;
}
...
bool materialScatter(vec3 throughput, HitRecord rec, Ray r,out Ray nextR, out vec3 atten, inout vec3 directLight)
{
    if(rec.matType == MAT_DIFFUSE)
		...
		return  DielectricScatter(r, rec, nextR, atten);
	else
		return false;
}
...
{   
    ...
    {
        vec3 v=normalize(position-mainCamera.position);
        float ni_over_nt=1.0/ior_glass;
        float cosine=-dot(v,normal);  

        vec3 refractRay=refract(v,normal,ni_over_nt);
        float reflect_prob;
    ...
    else if(color_mat.a == MAT_SPECULAR)
    {
