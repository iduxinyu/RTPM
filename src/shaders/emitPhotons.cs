...
float GetRandomNumber1D2(float idx)
{
    const float g  = 1.32471795724474602596f;
    const float a1 = 1.0f / (g * g);

    float x = 0.5f + a1 * (idx + 1.0f);
    return fract(x);
}

vec2 GetRandomNumber2D(float idx)
{
    const float g = 1.32471795724474602596f;
    const vec2 a = vec2(1.0f / g, 1.0f / (g * g));

    vec2 x = vec2(0.5f) + a * (idx + 1.0f);
    return fract(x);
}
                  ...
                  out uint sampleIdx)
{
    ...
    for(int mip=0; mip<=mipDepth; mip++)
    {
    ...
    out uint isReflective)
{
    ...
    vec2 RandomOffset=GetRandomNumber2D(float(taskIdx));
    ...
    if(ifHit(ray, rec))
    {
        ...
        else if(matType == MAT_SPECULAR || matType == MAT_REFRACT)
        {
            ...
            else
            {
                ...
                float transparency = 0.0; // 1 is opacity， 0 is full transparent
